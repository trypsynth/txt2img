...
const Cli = struct {
    ...
    fn parse(io: std.Io, allocator: std.mem.Allocator, raw_args: std.process.Args) !Cli {
        ...
        while (args.next()) |arg| {
            ...
            } else if (std.mem.eql(u8, arg, "-F") or std.mem.eql(u8, arg, "--fit")) {
                cli.fit = true;
            } else if (std.mem.eql(u8, arg, "--padding")) {
                const value = args.next() orelse return error.MissingValue;
                cli.padding = try std.fmt.parseInt(u32, value, 10);
            } else if (std.mem.eql(u8, arg, "--shadow")) {
                const value = args.next() orelse return error.MissingValue;
                cli.shadow = try parseShadow(value);
        ...
        if (cli.fit) {
            ...
            if (!seen_pos) {
                cli.pos_x = cli.padding;
                cli.pos_y = cli.padding;
            }
        }
        return cli;
    }
...
fn parseShadow(raw: []const u8) !Shadow {
    const sep = std.mem.indexOfScalar(u8, raw, ',') orelse return error.InvalidShadow;
    const dx = try std.fmt.parseInt(i32, raw[0..sep], 10);
    const dy = try std.fmt.parseInt(i32, raw[sep + 1 ..], 10);
    return .{ .dx = dx, .dy = dy };
}
